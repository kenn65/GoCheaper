...
**Email templates** are HTML files in `Templates/` built as `<EmbeddedResource>` with `<WithCulture>false</WithCulture>`. The `WithCulture=false` is **critical**: without it, MSBuild's `AssignCulture` task recognizes `da`, `nb`, `sv` as .NET culture codes and routes files like `AuthCodeEmail.da.html` into satellite assemblies (`da/*.resources.dll`) instead of the main assembly. `Assembly.GetManifestResourceStream` on the main assembly then returns null and every email falls back to English silently. `TemplateRenderer` replaces `{{Token}}` placeholders. Add a template by adding an `.html` file — the csproj glob picks it up automatically.

**Multilingual email templates:** `TemplateRenderer.Render(templateName, tokens, language)` first tries to load `{TemplateName}.{lang}.html`; if the exact match is not found it strips the region suffix and retries (e.g. `"da-DK"` → tries `"da"`); if still not found it falls back to the base `{TemplateName}.html` (English). Translated templates follow the naming convention `AuthCodeEmail.da.html`, `BookingReceiptEmail.sv.html`, etc. Supported languages: `en` (base), `da` (Danish), `nb` (Norwegian Bokmål), `sv` (Swedish). Language is threaded through Kafka events:
...
**Language detection in the Blazor circuit — CSP gotcha:** `IdentityApiClient` and `BookingApiClient` inject `IJSRuntime` and call `ResolveLanguageAsync()` before sending email-triggering requests. The implementation calls `js.InvokeAsync<string>("GoCheaper.getCultureLanguage")` — a named function defined in an inline `<script>` block in `App.razor`. **Never use `js.InvokeAsync<string>("eval", "...")`** for language detection: Blazor Server's Content Security Policy blocks `eval`, the call throws `JSException` which is silently swallowed, and the language falls back to `"en"`. A named function in a loaded script is not restricted by the `eval` CSP directive. `Routes.razor.OnAfterRenderAsync` uses the same named function as a safety net if `PersistentComponentState` didn't deliver the culture.

**`PreferredLanguage` sync on login:** `LoginHandler` updates `user.PreferredLanguage` and publishes `UserProfileUpdatedEvent` whenever a non-English `X-Language` header is received and the stored value differs. This keeps `DriverSnapshot.Language` in both Trips.Api and Booking.Api up to date so that booking and cancellation emails sent to a driver always arrive in the driver's own language. Without this, a driver whose `PreferredLanguage` was stored as `"en"` (e.g. registered before language detection was fixed) would receive all notification emails in English regardless of their actual language.
...
**`CultureContext` (Scoped):** Stores the active culture name (`"da"`, `"nb"`, `"sv"`, `"en"`) for the lifetime of a Blazor Server circuit. Set by `Routes.razor` in `OnInitializedAsync` using two sources in order: (1) **primary** — `IHttpContextAccessor.HttpContext.Features.Get<IRequestCultureFeature>().RequestCulture.UICulture.Name`, which reads the culture that `RequestLocalizationMiddleware` resolved for the original HTTP request (reliable because Blazor Server captures and holds the initial `HttpContext` for the circuit lifetime); (2) **fallback** — `PersistentComponentState.TryTakeFromJson<string>("culture")` written by `App.razor.PersistCulture()` during SSR. Read by `IdentityApiClient` and `BookingApiClient` when adding the `X-Language` header to API requests. `Program.cs` must call `builder.Services.AddHttpContextAccessor()` for `IHttpContextAccessor` to be injectable. **Do not use `CultureInfo.CurrentUICulture.Name` for X-Language headers in Blazor Server** — it is set thread-locally in `OnInitializedAsync` but Blazor dispatches event handlers in a separate async context that does not inherit that value, causing event-triggered API calls to silently send `"en"` regardless of the active UI language. `CultureContext` is the reliable alternative.

**`Routes.razor` culture restoration (three-layer approach):** `Routes.razor` is the interactive component (has `@rendermode="InteractiveServer"`), so its `RegisterOnPersisting` callback reliably fires during the SSR→circuit transition. `App.razor` is always SSR-only — its `RegisterOnPersisting` never fires. The restoration sequence in `OnInitializedAsync`: (1) register `PersistCulture` callback that writes `CultureInfo.CurrentUICulture.Name` under key `"routes-culture"`; (2) `TryTakeFromJson("routes-culture")` — succeeds in circuit phase with the value persisted during SSR; (3) `OnAfterRenderAsync(firstRender)` JS fallback — if `CultureCtx.Culture` is still `"en"`, reads the `AspNetCore.Culture` cookie and `navigator.language` via inline JavaScript interop as a guaranteed-reliable safety net.

**Email language resolution (Login/ForgotPassword handlers):** Non-English `X-Language` headers take priority (they reflect the user's active session language). An `"en"` header is treated as potentially a broken-CultureContext fallback, so `user.PreferredLanguage` from the DB takes precedence over it. Resolution order: `(X-Language if non-English) ?? user.PreferredLanguage ?? X-Language ?? "en"`. This means Danish users who have `PreferredLanguage = "da"` stored (set at registration or via the language switcher) receive Danish emails even if the header is wrong.
